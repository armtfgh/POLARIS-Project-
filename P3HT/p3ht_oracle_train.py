#!/usr/bin/env python
# aldol_oracle_train.py (patched: +LightGBM, +XGBoost)
import json
...
def make_model(kind: str, random_state=0):
    ...
    if k in ("lgbm", "lightgbm", "lgb"):
        if LGBMRegressor is None:
            raise ImportError("LightGBM not available. Please `pip install lightgbm`.")
    ...
    if k in ("xgb", "xgboost"):
        if XGBRegressor is None:
            raise ImportError("XGBoost not available. Please `pip install xgboost`.")
        return XGBRegressor(
            n_estimators=1000,
            learning_rate=0.05,
            subsample=0.8,
    ...
    raise ValueError("model must be one of: rf, gbr, mlp, lgbm, xgb")

def train(data_csv: str, out_dir: str, model="rf", test_size=0.2, random_state=0, cv_folds=5):
    df = pd.read_csv(data_csv)
    feat_cols = [c for c in df.columns if c != 'Conductivity']
    assert 'Conductivity' in df.columns and len(feat_cols) >= 2, "Expected 'Conductivity' and at least 2 features"
    df = df.dropna(subset=feat_cols + ['Conductivity']).reset_index(drop=True)
    X = df[feat_cols].copy()
    y = df['Conductivity'].astype(float).values

    # For simplicity we keep scaling for all models; trees don't require it but it's harmless.
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out_dir", default="p3ht_oracle_out")
    ap.add_argument("--model", default="rf", choices=["rf","gbr","mlp","lgbm","xgb"])
