...
class ContinuousVar:
    ...
    def normalize(self, x): lo, hi = float(self.low), float(self.high); return (np.asarray(x, float)-lo)/(hi-lo+1e-12)
...
class Schema:
    ...
    def ensure_df(self, X: pd.DataFrame) -> pd.DataFrame:
        miss=set(self.all_names())-set(X.columns)
...
class HeuristicReadout:
    def produce(self, schema: Schema, context_bullets=None):
        ...
        for v in schema.continuous:
            e,s,c=base(v.role or v.name.lower()); effects[v.name]={"effect":e,"scale":float(s),"confidence":float(c)}
        for c in schema.categorical:
            cs=c.choices; cat_sim[c.name]={a:{b:0.5 for b in cs if b!=a} for a in cs}
...
class PriorBuilder:
    ...
    def _m0_cont(self, var: ContinuousVar, z: np.ndarray):
        eff=(self.readout.get("effects",{}) or {}).get(var.name,{})
        effect=eff.get("effect","flat"); scale=float(eff.get("scale",0.0)); conf=float(eff.get("confidence",0.0)); rh=eff.get("range_hint",None)
        mu=0.6
    ...
    def m0_df(self, X: pd.DataFrame) -> np.ndarray:
        ...
        # simple interaction term (if declared)
        inters=(self.readout.get("interactions") or [])
        name_to_norm={v.name:v.normalize(X[v.name].to_numpy(float)) for v in self.schema.continuous}
        for it in inters:
            pair=it.get("pair",[]); tp=it.get("type","synergy"); conf=float(it.get("confidence",0.3))
            if isinstance(pair,(list,tuple)) and len(pair)==2 and all(p in name_to_norm for p in pair):
                term=name_to_norm[pair[0]]*name_to_norm[pair[1]]
                contrib += (0.2*conf)*(1.0 if tp=="synergy" else -1.0)*term
        return 100.0*contrib

    def phi_df(self, X: pd.DataFrame, degree:int=3) -> np.ndarray:
...
class ResidualizedRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_estimator: RegressorMixin, prior: PriorBuilder, use_base_vars: bool=True):
        ...
        self._scaler=StandardScaler(with_mean=True, with_std=True); self._fitted_estimator=None
    def _build(self, X):
    ...
    def fit_df(self, X: pd.DataFrame, y: np.ndarray):
        Xaug=self._build(X); m0=self.prior.m0_df(X); yres=np.asarray(y,float)-m0; self._fitted_estimator=clone(self.base_estimator); self._fitted_estimator.fit(Xaug,yres); self.fitted_=True; return self
    def predict_df(self, X: pd.DataFrame)->np.ndarray:
        Xaug=self._build(X); yres=self._fitted_estimator.predict(Xaug); return self.prior.m0_df(X)+yres
